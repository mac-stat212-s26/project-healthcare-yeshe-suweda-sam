...
[Slideshow](https://docs.google.com/presentation/d/1wqG2V_0zz8rm9PhzIilINZWCDRqh-YvqoaYG-aTcGZE/edit?usp=sharing)

[Video](https://drive.google.com/file/d/19dHClFmv2kI7pohqrpJVFZLzoMLcM8CS/view?usp=drive_link)
...
To answer our research questions, we looked at datasets from the [Minnesota Department of Health](https://mndata.web.health.state.mn.us/birth_defects/messaging_pre.html).
...
To begin this investigation, it's crucial to understand the scale of the problem.
...
prevalence <- maternal_master %>%
  group_by(Note) %>%
  summarize(total_count = sum(Count))

ggplot(prevalence, aes(y = reorder(Note, total_count), x = total_count, fill = Note)) +
  geom_bar(stat = 'identity') +
  labs(title = "General Prevalence of Birth Defect Types", x = "Count", y = "Defect type") +
  scale_fill_viridis_d(guide = "none") +
  theme_classic()

```
Cardiovascular defects are by far the most prevalent, more than doubling the next most frequent, genitourinary. According to [Mayo Clinic](https://www.mayoclinic.org/diseases-conditions/congenital-heart-defects-children/symptoms-causes/syc-20350074), these defects create an abnormality in the structure or function of the heart. Their exact cause isn't certain, but researchers note that medicines, health conditions, or lifestyles may be related. 
...
Both of the dimensions on education point in the same direction, suggesting a compounding disadvantage. Mothers who are both lower-income and less educated face the greatest barriers to consistent folic acid intake — and therefore the highest exposure to preventable NTD risk.
...
When looking at the birth defect outcomes by maternal race/ethnicity in Minnesota (2016–2020), the disparities reflect significant inequalities. 
```{r echo = FALSE}
